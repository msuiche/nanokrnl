//! End-to-end test of the kernel-authored ELF core dump: boot, type `crash`, and
//! run an in-process writable 9P server that captures the `nanokrnl.core` the
//! kernel streams to `H:\`. Writes it to /tmp/nanokrnl.core for inspection.
//!
//!   cargo run --release --example core_dump -- ../target/x86_64-unknown-none/debug/kernel
...
fn main() {
    ...
    let base = out.len();
    // Small slices: the client spins for each Twrite reply until the slice ends
    // and we pump, so a big slice wastes cycles. Small slices keep it moving.
    for i in 0..200_000 {
        let st = m.run(300_000);
        for b in m.take_uart_output() { out.push(b as char); }
        srv.pump(&mut m);
        if srv.core_done { break; } // the core file was fully written + clunked
        if i % 2000 == 0 {
            if let Some(f) = srv.files.get("nanokrnl.core") {
                eprint!("\r[core_dump] {} KB written...", f.len() / 1024);
            }
        }
    ...
    match srv.files.get("nanokrnl.core") {
        Some(core) => {
            std::fs::write("/tmp/nanokrnl.core", core).unwrap();
            println!("wrote /tmp/nanokrnl.core ({} bytes)", core.len());
            assert!(&core[0..4] == b"\x7fELF", "not an ELF");
            println!("PASS: kernel wrote an ELF core to H:\\ over 9P");
        }
        None => panic!("kernel did not write nanokrnl.core"),
    }
}
...
struct P9Server {
    ...
    wname: Option<String>, // the fid opened for write (FILE_FID)
    served: u32,
    core_done: bool, // set when nanokrnl.core is fully written + clunked
}
impl P9Server {
    fn new() -> Self { P9Server { inbuf: Vec::new(), files: HashMap::new(), wname: None, served: 0, core_done: false } }
    ...
    fn serve(&mut self, msg: &[u8]) -> Vec<u8> {
        ...
        match typ {
            ...
            110 => { // Twalk fid newfid nwname names -> clone (nwname=0)
            ...
            14 => { // Tlcreate fid name flags mode gid -> Rlcreate qid iounit
            ...
            118 => { // Twrite fid offset count data -> Rwrite count
            ...
            120 => { // Tclunk: if the write fid was nanokrnl.core, it's now complete.
                if self.wname.as_deref() == Some("nanokrnl.core") { self.core_done = true; }
                self.wname = None; R::new(121, tag).done()
            }
            _ => { let mut r = R::new(7, tag); r.u32(22); r.done() } // Rlerror EINVAL
        }
    }
}
struct R(Vec<u8>);
