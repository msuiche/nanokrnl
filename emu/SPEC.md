# nanox — bespoke x86-64 emulator specification

`nanox` (crate in `emu/`) is a hand-written x86-64 **full-machine** emulator
...
`nanox` instead emulates **only what our kernel needs** and **boots directly in
...
(`web/nanox/index.html`) drives it.

| Export | Meaning |
|---|---|
| `nanox_new(ram_mb)` | Create the machine with N MiB of RAM. |
| `nanox_image_alloc(len) -> ptr` | Reserve an image buffer; JS writes bytes into wasm memory at `ptr`. |
| `nanox_boot_elf(rsp) -> entry` | Parse the staged image as ELF, load it, boot long mode at its entry. |
| `nanox_boot(entry, rsp)` | Boot long mode at a raw entry. |
| `nanox_set_idt_gate(vector, handler)` | Install a 64-bit interrupt gate. |
| `nanox_run(steps) -> code` | Run; `0` halted, `1` max-steps, `2` unknown-opcode, `3` unhandled-fault, `4` syscall-trap. |
| `nanox_uart_read() -> i32` | Pop a UART output byte, or `-1`. |
| `nanox_uart_write(byte)` / `nanox_key(scancode)` | Feed console / keyboard input. |
...
# Browser build (wasm32, no_std) + stage into web/nanox/:
sh emu/build-wasm.sh
(cd web/nanox && python3 -m http.server 8000)   # open http://localhost:8000

# Inspect / attempt to boot the real kernel ELF under nanox:
...
**The real ntoskrnl-rs kernel boots** under nanox (see §7) — natively
...
`nanox.wasm`, driven through its JS ABI, loads a synthetic ELF, boots long mode,
...
dispatcher context-switches onto the created system thread. nanox models this
...
nanox is validated against authoritative oracles rather than by hand — this is
...
- **Decode/length** (`examples/conformance.rs`): disassembles the kernel `.text`
  with **iced-x86** and checks nanox decodes each instruction to the same
